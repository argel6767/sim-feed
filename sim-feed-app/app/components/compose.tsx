...
const ErrorCompose = ({ message, onRetry, onDismiss }: ErrorComposeProps) => {
  return <Error message={message} onRetry={onRetry} onDismiss={onDismiss} />;
};
    ...
    <div className="bg-sf-bg-card border border-sf-border-primary rounded-lg p-6 mb-4 motion-preset-fade motion-delay-100 max-h-90">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          ...
          <Link
            className="text-sf-text-dim text-[0.85rem]"
            to={`/users/${user?.id}`}
          >
            @{user?.username}
          </Link>
      ...
      <input
        type="text"
        className="w-full bg-sf-bg-primary border border-sf-border-primary rounded p-4 text-sf-text-primary font-sans text-[1rem] resize-none transition-colors duration-300 focus:outline-none focus:border-sf-accent-primary mb-2"
...
export const CommentCompose = ({ postId }: CommentComposeProps) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      if (canComment) createNewComment();
    }
  };
  ...
  const retryCreateComment = async () => {
    setIdle();
    await createNewComment();
  };
  
  const isEmpty = commentText.length === 0;
  ...
  return (
    <div className="bg-sf-bg-card border border-sf-border-primary rounded-lg p-6 mb-4 motion-preset-fade motion-delay-100 max-h-90">
      ...
      <div className="flex justify-end gap-2 mt-4">
        <button
          className="px-5 py-2.5 border border-sf-border-subtle rounded text-[0.85rem] font-semibold uppercase tracking-[0.5px] bg-transparent text-sf-text-tertiary transition-all duration-300 hover:border-sf-text-secondary hover:text-sf-text-primary cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed disabled:pointer-events-none"
