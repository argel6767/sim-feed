...
describe('getPostComments Handler', () => {
  ...
  it('returns a posts comments', async () => {
      ...
      expect(JSON.parse(response.body!)).toEqual([{ id: 1, body: 'Test Comment', author: 1 }]);
    });
  
  it('should return a 400 when postId is not a number', async() => {
    const event = createEvent();
    event.pathParameters!.post_id = 'abc';
    const response: APIGatewayProxyStructuredResultV2 = await handler(event, context, callback) as APIGatewayProxyStructuredResultV2;
    expect(response.statusCode).toBe(400);
    expect(JSON.parse(response.body!)).toEqual({ error: 'Bad Request', message: 'Missing post_id parameter' });
  })
