import { handler } from "../src/handlers/addNewUser";
...
describe("addNewUser handler", () => {
  ...
  beforeEach(() => {
    ...
    process.env = {
      ...originalEnv,
      SIGNING_SECRET_INSERT: "test_signing_secret",
      USE_SSL: "false",
    };
  ...
  const createEvent = (
    ...
    version: "2.0",
    routeKey: `${method} /users/web-hook`,
    rawPath: "/users/web-hook",
    rawQueryString: "",
    headers: {
      "svix-id": "msg_test123",
  ...
  it("should return 200 with 'Event ignored' for non user.created events", async () => {
    const body = JSON.stringify({
      type: "user.updated",
  ...
  it("should return 201 and create user on valid user.created event", async () => {
  ...
  it("should throw when SSM parameter value is missing", async () => {
    ...
    await expect(handler(event, context, callback)).rejects.toThrow(
      "SIGNING_SECRET_INSERT not found in SSM",
    );
  });

  it("should use SIGNING_SECRET_INSERT env var when USE_SSL is false", async () => {
    process.env.USE_SSL = "false";
    process.env.SIGNING_SECRET_INSERT = "env_secret";
